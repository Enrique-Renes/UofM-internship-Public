...
NAs for variables in TREE, PLOTSNAP and COND tables:
```{r}
...
TN_tree_stat_summary <- cbind(a,b,c,d,e,f,g,h,i,j,l,m)
colnames(TN_tree_stat_summary) <- c("Inventory year","Plot","Status code","Species code","Species group code","Current diameter","Total height","Actual height","Tree class code","Death cause agent code","Tree/acre/year unadjusted","Above ground dry biomass")
...
Violin plots for Tree table variables:
```{r echo=FALSE}
...
Violin plots for plotsnap table variables:
```{r echo=FALSE}
...
Violin plots for Cond table variables:
```{r echo=FALSE}
...
Plotsnap table correlation matrix:
```{r}
...
Cond table correlation matrix: 
```{r}
#For some reason I cannot understand, this chunk won't work
